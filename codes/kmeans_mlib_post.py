""" Clustering on Posts using Mlib Package """
import sys
import numpy as np
...
if __name__ == "__main__":
	sc = SparkContext(appName="kmeans_user")
	...
	clusters = KMeans.train(datardd1, 5, maxIterations=100, initializationMode="random")
	...
	plotResults.saveAsTextFile("costForEachKMlibPost")		

	print("Within Set Sum of Squared Error = " + str(WSSSE))
	# Predict the cluster index and distance for each userID and store the result
	datardd2 = datardd1.map(lambda x: (clusters.predict(x), x))	
	datardd2.saveAsTextFile("stackOverflowPostOutputMlib")
